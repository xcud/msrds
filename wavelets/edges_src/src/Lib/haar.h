...
private:
        ...
        inline void addsub(__m64 even, __m64 odd,                                   //addsub for transform
                           __m64 *lo, __m64 *hi, unsigned int t = 8) const;        //denoise lo from [t ... 8)
        inline char threshold(int c) const;                                         //if(abs(c)<TH)c=0
...
inline void Haar::addsub(__m64 even, __m64 odd, __m64 *lo, __m64 *hi, unsigned int t) const      //even,odd - chars,  lo,hi - chars
{
        char *clo = (char *)lo;
        char *chi = (char *)hi;
        ...
        *hi = mb;
        ...
        for (unsigned int i = 0; i < 8; i++) {
                chi[i] = threshold(chi[i]);
                if (i >= t)
                        clo[i] = threshold(clo[i]);
        }
}

inline char Haar::threshold(int c) const
{
        int th = (int)getTH();
