...
#include "cvLib\motiondetector.h"
...
int cvDetect(const unsigned char *pBGR)
{
        ...
        __try {
                resize.resize(pBGR);        
                return fdetect.detect(resize.gety(), resize.getr(), resize.getg(), resize.getb(), mdetect.detect(resize.gety(), &fdetect));                
        }
        __except(EXCEPTION_EXECUTE_HANDLER) {
                swprintf(err, 256, L"SEH error code: 0x%X", GetExceptionCode());
...
{
        return fdetect.get_motion_amount();
}
