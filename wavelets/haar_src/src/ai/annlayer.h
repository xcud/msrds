...
public:
        ...
        void set_input(const vec2D& x, enum ACTFUN fun);                    //set this->inputs[1:size-1] = actfun( x );   inputs[0]-bias 1
        void get_output(float* ovec, enum ACTFUN fun) const;                //ovec = actfun(this->outputs)

        void run(enum ACTFUN fun, AnnLayer* pnext = 0);                     //outputs = inputs*W'
