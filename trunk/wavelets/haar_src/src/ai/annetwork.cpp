...
void ANNetwork::classify(const float* ivec, float* ovec) const
{
        ...
        *m_input_vec = ivec;
        ...
        m_layers[0]->set_input(*m_input_vec, m_actfuns[0]);         //copy m_input_vec to layer[0]->inputs[1:size-1],  0-bias
        ...
        for (unsigned int l = 0; l < m_layers.size(); l++) {
                ...
                if (l + 1 < m_layers.size())
                        pnext = m_layers[l + 1];
                m_layers[l]->run(m_actfuns[l + 1], pnext);
        }

        //output layer
        m_layers[m_layers.size() - 1]->get_output(ovec, m_actfuns[m_layers.size() - 1]);
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
